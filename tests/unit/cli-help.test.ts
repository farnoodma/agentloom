...
describe("cli help routing", () => {
  it("prints top-level help", async () => {
    ...
    expect(output).toContain("dotagents <command> [options]");
  ...
  it("prints mcp help without requiring scope resolution", async () => {
    ...
    const output = logSpy.mock.calls.map((call) => String(call[0])).join("\n");
    expect(output).toContain("dotagents mcp <command> [options]");
  ...
  it("throws actionable unknown command message", async () => {
    await expect(runCli(["unknown"])).rejects.toThrow("dotagents --help");
  });
});
