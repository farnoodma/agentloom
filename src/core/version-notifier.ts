...
export async function maybeNotifyVersionUpdate(
  ...
  if (process.env.DOTAGENTS_DISABLE_UPDATE_NOTIFIER === "1") return;
  ...
  const packageName = options.packageName ?? "dotagents";
...
function printNotice(current: string, latest: string): void {
  console.error(
    `\nUpdate available for dotagents: ${current} -> ${latest}\nRun: npm i -g dotagents\n`,
  );
}
